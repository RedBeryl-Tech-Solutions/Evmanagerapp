...
class SubscriptionDetailsResourceIT {
    ...
    private static final ZonedDateTime DEFAULT_SUBSCRIPTION_START_DATE = ZonedDateTime.ofInstant(Instant.ofEpochMilli(0L), ZoneOffset.UTC);
    private static final ZonedDateTime UPDATED_SUBSCRIPTION_START_DATE = ZonedDateTime.now(ZoneId.systemDefault()).withNano(0);

    private static final ZonedDateTime DEFAULT_SUBSCRIPTION_EXPIRY_DATE = ZonedDateTime.ofInstant(Instant.ofEpochMilli(0L), ZoneOffset.UTC);
    private static final ZonedDateTime UPDATED_SUBSCRIPTION_EXPIRY_DATE = ZonedDateTime.now(ZoneId.systemDefault()).withNano(0);
    ...
    void getAllSubscriptionDetails() throws Exception {
        ...
        restSubscriptionDetailsMockMvc
            ...
            .andExpect(jsonPath("$.[*].subscriptionStartDate").value(hasItem(sameInstant(DEFAULT_SUBSCRIPTION_START_DATE))))
            .andExpect(jsonPath("$.[*].subscriptionExpiryDate").value(hasItem(sameInstant(DEFAULT_SUBSCRIPTION_EXPIRY_DATE))))
    ...
    void getSubscriptionDetails() throws Exception {
        ...
        restSubscriptionDetailsMockMvc
            ...
            .andExpect(jsonPath("$.subscriptionStartDate").value(sameInstant(DEFAULT_SUBSCRIPTION_START_DATE)))
            .andExpect(jsonPath("$.subscriptionExpiryDate").value(sameInstant(DEFAULT_SUBSCRIPTION_EXPIRY_DATE)))
